...
# bom_id, name, category, price, opening balance, safety stock, reorder qty
BOMS = [
    ("BOM-001", "Steel bracket 40mm",   "Component",     12.50, 120, 30, 60),
    ("BOM-002", "M6 hex bolt (100 pk)", "Component",      4.20,  25, 40, 80),   # already low
    ("BOM-003", "Corrugated box L",     "Packaging",      0.90, 500, 100, 200),
    ("BOM-004", "Aluminium sheet 2mm",  "Raw Material",  38.00,  15, 10, 20),
    ("BOM-005", "Machine oil 5L",       "Consumable",    22.00,   8,  8, 16),   # exactly at safety stock
]
...
def seed():
    ...
    for bom_id, name, cat, price, bal, safety, reorder in BOMS:
        ...
        db.execute(
            """INSERT INTO bom (bom_id, bom_name, category, price, opening_balance, current_balance,
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (bom_id, name, cat, price, bal, bal, safety, reorder, admin, now()))
